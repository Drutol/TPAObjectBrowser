...
namespace ObjectBrowser.Test.StorageTest
{
    ...
    public class DatabaseStorageTest
    {
        ...
        {
            var storage = new DatabaseDataStorage();
            await storage.Save(_metadata);
            var data = await storage.Retrieve();

            CollectionAssert.AreEqual(data.RegisteredTypes.Select(metadata => metadata.TypeName).OrderBy(s => s).ToList(),
                _metadata.RegisteredTypes.Select(metadata => metadata.TypeName).OrderBy(s => s).ToList());
        }
    }
}
